...
- quantitative phase image background correction, or
- extraction of refractive index and radius for spherical phase objects
  such as liquid droplets, microgel beads, or biological cells. 
  

Quantitative phase imaging
--------------------------
Quantitative phase imaging (QPI) is an imaging technique that quantitatively
measures the phase retardation of a wave that traveled through a specimen.
...
and wavelength [nm]), but the concepts apply to both methods.
...
The first assumption of dry mass computation in QPI is that :math:`f(x,y,z)`
...
with the RI of the surrounding medium :math:`n_\text{med}`. For a thorough discussion
on the relationship between mass density and RI, see reference :cite:`Barer_1954`.
In QPI, the RI is measured indirectly as a projected quantitative phase retardation
image :math:`\phi(x,y)`.
...
Integrating the above equations over the detector area yields
...
default, an approximate average value used in literature :cite:`Popescu_2008`,
:cite:`Schuermann_2016`. The refraction increment can be manually modified using
the configuration key "refraction increment" in the "sphere" section.